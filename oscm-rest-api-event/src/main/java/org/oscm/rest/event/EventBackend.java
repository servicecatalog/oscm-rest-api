...
public class EventBackend {
    ...
    public RestBackend.Post<EventRepresentation, EventParameters> post() {
        return new RestBackend.Post<EventRepresentation, EventParameters>() {
            ...
            @Override
            public Object post(EventRepresentation content,
                    EventParameters params) throws Exception {
                if (content.isSubcriptionKeySet()) {
                    es.recordEventForSubscription(content.getSubscriptionKey(),
                            content.getVO());
                } else {
                    es.recordEventForInstance(content.getTechnicalServiceId(),
                            content.getInstanceId(), content.getVO());
                }
                return null;
            }
        };
