...
public class UsageLicenseBackend {
    ...
    public RestBackend.Post<UsageLicenseRepresentation, SubscriptionParameters> post() {
        return new RestBackend.Post<UsageLicenseRepresentation, SubscriptionParameters>() {
            ...
            public Object post(UsageLicenseRepresentation content,
                ...
                VOSubscriptionDetails sub = ss.getSubscriptionDetails(params
                        .getId().longValue());
                boolean added = ss.addRevokeUser(sub.getSubscriptionId(),
                        Collections.singletonList(content.getVO()), null);
                Long licKey = null;
                if (added) {
                    List<VOUsageLicense> lics = ss.getSubscriptionDetails(
                            params.getId().longValue()).getUsageLicenses();
                    for (VOUsageLicense lic : lics) {
                        if (lic.getUser().getUserId()
                                .equals(content.getUser().getUserId())) {
    ...
    public RestBackend.GetCollection<UsageLicenseRepresentation, SubscriptionParameters> getCollection() {
        return new RestBackend.GetCollection<UsageLicenseRepresentation, SubscriptionParameters>() {
            ...
            public RepresentationCollection<UsageLicenseRepresentation> getCollection(
                ...
                VOSubscriptionDetails sub = ss.getSubscriptionDetails(params
                        .getId().longValue());
                List<UsageLicenseRepresentation> lics = UsageLicenseRepresentation
                        .convert(sub.getUsageLicenses());
            ...
            public boolean delete(SubscriptionParameters params)
                ...
                for (VOUsageLicense lic : lics) {
                    if (lic.getKey() == licKey) {
                        result = ss.addRevokeUser(sub.getSubscriptionId(),
                                null, Collections.singletonList(lic.getUser()));
                        break;
                    }
                }
                return result;
