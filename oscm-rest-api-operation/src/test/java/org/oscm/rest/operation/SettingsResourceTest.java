...
class SettingsResourceTest {
    ...
    @Mock
    private SettingsBackend settingsBackend;
    ...
    public void shouldGetSettings() {
        when(settingsBackend.getCollection()).thenReturn(
                operationParameters -> new RepresentationCollection<>(
                        Lists.newArrayList(settingRepresentation)));
        ...
        assertThat(response).extracting(r -> {
    ...
    public void shouldCreateSettings() {
        try {
            when(settingsBackend.post()).thenReturn(
                    (settingRepresentation1, operationParameters) -> 100L);
        } catch (Exception e) {
            fail(e);
        }

        try {
            response = settingsResource.createSetting(uriInfo, settingRepresentation, operationParameters);
        } catch (Exception e) {
            fail(e);
        }
    ...
    public void shouldUpdateSettings() {
        try {
            when(settingsBackend.put()).thenReturn(
                    (settingRepresentation1, operationParameters) -> true);
        } catch (Exception e) {
            fail(e);
        }

        try {
            response = settingsResource.updateSetting(uriInfo, settingRepresentation, operationParameters);
        } catch (Exception e) {
            fail(e);
        }
