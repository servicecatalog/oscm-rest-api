...
public class RolesResourceTest {
        ...
        public void shouldGetUserRoles() {
                when(userBackend.getRoles())
                        .thenReturn(userParameters -> rolesRepresentation);

                try {
                        result = rolesResource
                                .getUserRoles(uriInfo, parameters);
                } catch (Exception e) {
                        fail(e);
                }
                ...
                assertThat(result).extracting(Response::hasEntity)
                        .isEqualTo(true);
                assertThat(result).extracting(Response::getEntity)
                        .isEqualTo(rolesRepresentation);
        }

        @Test
        public void shouldSetUserRoles() {
                when(userBackend.putRoles()).thenReturn(
                        (rolesRepresentation1, userParameters) -> true);

                try {
                        result = rolesResource
                                .setUserRoles(uriInfo, rolesRepresentation,
                                        parameters);
                } catch (Exception e) {
                        fail(e);
                }
