...
public class UserResourceTest {
                        ...
                        .extracting(
                                r -> {
                                        RepresentationCollection<UserRepresentation> representationCollection =
                                                (RepresentationCollection<UserRepresentation>) r
                                                        .getEntity();
        ...
        public void shouldCreateUser() {
                when(userBackend.postUser()).thenReturn(
                        (userRepresentation1, userParameters) -> "newId");

                try {
                        result = userResource
                                .createUser(uriInfo, userRepresentation,
                                        parameters);
                } catch (Exception e) {
                        fail(e);
                }
        ...
        public void shouldGetSingleUser() {
                when(userBackend.getUser())
                        .thenReturn(userParameters -> userRepresentation);
        ...
        public void shouldUpdateUser() {
                when(userBackend.putUser()).thenReturn(
                        (userRepresentation1, userParameters) -> true);

                try {
                        result = userResource
                                .updateUser(uriInfo, userRepresentation,
                                        parameters);
                } catch (Exception e) {
                        fail(e);
                }
