...
public class BillingContactResourceTest {
        ...
        @InjectMocks
        @Spy
        private BillingContactResource resource;
        ...
        public void shouldGetBillingContacts() {
                ...
                try {
                        result = resource
                                .getBillingContacts(uriInfo, parameters);
                } catch (Exception e) {
                        fail(e);
                }
                        ...
                        .extracting(
                                r -> {
                                        RepresentationCollection<BillingContactRepresentation> collection =
                                                (RepresentationCollection<BillingContactRepresentation>) r
                                                        .getEntity();
        ...
        public void shouldCreateBillingContact() {
                when(accountBackend.postBillingContact())
                        .thenReturn(
                                ((billingContactRepresentation, accountParameters) -> "newId"));

                try {
                        result = resource
                                .createBillingContact(uriInfo, representation,
                                        parameters);
                } catch (Exception e) {
                        fail(e);
                }
        ...
        public void shouldGetSingleBillingContact() {
                when(accountBackend.getBillingContact())
                        .thenReturn((accountParameters -> representation));

                try {
                        result = resource
                                .getBillingContact(uriInfo, parameters);
                } catch (Exception e) {
                        fail(e);
                }
        ...
        public void shouldUpdateBillingContact() {
                when(accountBackend.putBillingContact())
                        .thenReturn(
                                ((billingContactRepresentation, accountParameters) -> true));

                try {
                        result = resource
                                .updateBillingContact(uriInfo, representation,
                                        parameters);
                } catch (Exception e) {
                        fail(e);
                }
        ...
        public void shouldDeleteBillingContact() {
                when(accountBackend.deleteBillingContact())
                        .thenReturn((accountParameters -> true));

                try {
                        result = resource
                                .deleteBillingContact(uriInfo, parameters);
                } catch (Exception e) {
                        fail(e);
                }
